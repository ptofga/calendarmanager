...
class WeChat(WeChatBase):
    ...
    def rollToTop(self):
        ...
        self.C_MsgList.SendKeys('{Home}', waitTime=1)  # 按键盘上的“Home”键，跳转至窗口顶部

        children = self.C_MsgList.GetChildren()
        # 遍历所有子元素
    ...
    def GetAllSessionList(self):
        sessions_list = []
        prev_count = 0

        while True :
            current_sessions=self.GetSession()
            name_list = [i.name for i in current_sessions]
            prev_count = len(sessions_list)
            sessions_list.extend(s for s in name_list if s not in sessions_list) 
            #print(f"{prev_count}-----{len(sessions_list)}")
            #print(sessions_list)
            if len(sessions_list) == prev_count:
                 break  # 停止条件：无新增会话            
            self.SessionBox.WheelDown(wheelTimes=8, interval=0)
